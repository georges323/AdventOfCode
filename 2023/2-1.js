function sumOfGameIds(gameInput, redCubeLimit, greenCubeLimit, blueCubeLimit) {
    var games = gameInput.split("\n");

    var sum = 0;

    var gameIdToData = new Map();

    for (let i = 0; i < games.length; i++) {
        var gameLineSplit = games[i].split(":");

        var gameStr = gameLineSplit[0];
        
        var gameStrSplit = gameStr.split(" ");
        var gameId = gameStrSplit[1].substring(0, gameStrSplit[1].length);
        

        var gameData = gameLineSplit[1];
        var gameDataSplit = gameData.split(";").map(x => x.trim());

        gameIdToData.set(parseInt(gameId), gameDataSplit);
    }

    for (const [id, gameData] of gameIdToData.entries()) {
        var possibleGame = true;
        
        for (let data of gameData) {
            if (possibleGame == false) break;

            var cubeSplit = data.split(",");

            for (let cubes of cubeSplit) {
                var cubeInfo = cubes.trim().split(" ");

                var numberOfCubes = parseInt(cubeInfo[0]);
                var cubeColor = cubeInfo[1];

                if ((cubeColor == 'blue' && numberOfCubes > blueCubeLimit) ||
                (cubeColor == 'red' && numberOfCubes > redCubeLimit)||
                (cubeColor == 'green' && numberOfCubes > greenCubeLimit)) {
                    possibleGame = false;
                }
            }
        }
...
var recordOfGameExample = `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`;

var actualGameInput = `Game 1: 7 green, 14 red, 5 blue; 8 red, 4 green; 6 green, 18 red, 9 blue
...
console.log(sumOfGameIds(actualGameInput, 12, 13, 14));