function sumOfCalibrationDocument(inputText) {
    var listOfSingleLines = inputText.split("\n");

    var sum = 0;
    var foundNumbers = [];

    for (var i = 0; i < listOfSingleLines.length; i++) {
        var line = listOfSingleLines[i];

        if (line.length == 0) continue;

        for (let i = 0; i <= line.length; i++) {
            if (!isNaN(line[i])) {
                foundNumbers.push(parseInt(line[i]));
                continue;
            }

            for (let j = i+2; j <= i+4; j++) {
                if (j >= line.length || !isNaN(line[j])) {
                    break;
                }
                var convertedNumber = convertToNumber(line.substring(i, j+1));

                if (convertedNumber == 0) {
                    continue;
                }

                foundNumbers.push(convertedNumber);
                break;
            }
        }
        ...
        if (foundNumbers.length == 1) {
            sum += foundNumbers[0]*10 + foundNumbers[0];
        } else {
            var lastIndex = foundNumbers.length-1;

            sum += foundNumbers[0]*10 + foundNumbers[lastIndex];
        }
...
function convertToNumber(numberLetter) {
    if (numberLetter == undefined ||
        numberLetter == null ||
        numberLetter.length == 0) return 0;
...
var actualInput = `76xkqjzqtwonfour
...
console.log(sumOfCalibrationDocument(actualInput));
